...
pub fn main() !void {
    if (!window.initHeadless(128, 96)) return error.HeadlessInitFailed;
    defer window.closeWindow();
    gfx.setScreenSize(128, 96);
    gfx.setDesignSize(128, 96);
    ...
    const bytes = try std.Io.Dir.cwd().readFileAlloc(io, "texture_sampling_probe.tga", std.heap.page_allocator, .limited(128 * 96 * 8));
    ...
    if (image.width != 128 or image.height != 96) return error.BadCaptureSize;
