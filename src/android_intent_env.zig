/// Launch-intent extras → environment variables (labelle-bgfx#139).
///
...
/// It has no Android dependency so it runs as a HOST unit test (build.zig);
/// the JNI read is `android_intent_extras.c`, the driver `android_app.zig`.
        ...
        switch (action(key, extra, allowed, state.set_by_intent[i])) {
            .set => |v| {
                ...
                if (!state.set_by_intent[i]) {
                    ...
                    if (env.get(key.name)) |orig| {
                        if (orig.len > max_original_len) {
                            std.log.info("bgfx: keeping {s} from the environment (too long to save); intent extra ignored", .{key.name});
                            continue;
                        }
                ...
                if (env.set(key.name, v)) {
                    ...
                    std.log.info("bgfx: {s}={s} (launch intent extra)", .{ key.name, v });
                }
            },
            .revert => {
                if (state.original(i)) |orig| {
                    _ = env.set(key.name, orig);
                    std.log.info("bgfx: {s} restored to {s} (set by a previous launch's intent)", .{ key.name, orig });
                } else {
                    env.unset(key.name);
                    std.log.info("bgfx: {s} cleared (set by a previous launch's intent)", .{key.name});
                }
                state.set_by_intent[i] = false;
            },
            .keep => if (extra != null and key.debuggable_only and !allowed and extra.?.len > 0) {
                std.log.info("bgfx: ignoring intent extra {s}: the apk is not debuggable", .{key.name});
            },
        }
    }
}
